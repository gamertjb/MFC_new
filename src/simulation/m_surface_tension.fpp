                    ...
                    do k = isy%beg, isy%end
                        do j = isx%beg, isx%end
                            ...
                            if (normW > capillary_cutoff) then
                                ...
                                flux_src_vf(E_idx)%sf(j, k, l) = flux_src_vf(E_idx)%sf(j, k, l) + &
                                                                 sigma_coeff*c_divs(num_dims + 1)%sf(j, k, l)*vSrc_rsx_vf(j, k, l, 1)
                            end if
                    ...
                    do k = isy%beg, isy%end
                        do j = isx%beg, isx%end
                            ...
                            if (normW > capillary_cutoff) then
                                ...
                                flux_src_vf(E_idx)%sf(j, k, l) = flux_src_vf(E_idx)%sf(j, k, l) + &
                                                                 sigma_coeff*c_divs(num_dims + 1)%sf(j, k, l)*vSrc_rsy_vf(k, j, l, 2)
                            end if
                        ...
                        do k = isy%beg, isy%end
                            do j = isx%beg, isx%end
                                ...
                                if (normW > capillary_cutoff) then
                                    ...
                                    flux_src_vf(E_idx)%sf(j, k, l) = flux_src_vf(E_idx)%sf(j, k, l) + &
                                                                     sigma_coeff*c_divs(num_dims + 1)%sf(j, k, l)*vSrc_rsz_vf(l, k, j, 3)
                                end if
                    ...
                    do k = 0, n
                        do j = 0, m
                            c2_divs(1)%sf(j, k, l) = 1._wp/(x_cc(j + 1) - x_cc(j - 1))* &
                                                     (q_prim_vf(c2_idx)%sf(j + 1, k, l) - q_prim_vf(c2_idx)%sf(j - 1, k, l))
                        end do
                    end do
                end do
            #:endcall GPU_PARALLEL_LOOP
